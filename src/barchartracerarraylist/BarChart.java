...
public class BarChart { 
    ...
    public void add(Bar bar) {
        if (bar.getName() == null) throw new IllegalArgumentException("name is null");
        if (bar.getCategory() == null) throw new IllegalArgumentException("category is null");

        if (!colorOf.containsKey(bar.getCategory())) {
            colorOf.put(bar.getCategory(), COLORS[colorOf.size() % COLORS.length]);
        }
        Color color = colorOf.get(bar.getCategory());
        String name = bar.getName();
        int value = bar.getValue();
        
        // *** Add "color" to the colors ArrayList.
        // *** Add "name" to the names ArrayList.
        // *** If "value" is 0 or greater, add it to the values ArrayList.
        // *** If not, add 0 to the values ArrayList.
     ...
     */
    public void draw() {
        // *** Set to the number of items in the names ArrayList.
        int numberOfBars = 0;  
        ...
        {    
            ...
            // *** Set xmax to the maximum value in the values ArrayList.
